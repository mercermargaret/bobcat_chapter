...
#SBATCH --array=0-32%33                      # set number of total simulations and number that can run simultaneously (0-33%34)
...
if [ -f results/results_rr.csv ]; then
    ...
    echo "name, days, real_crossings_maj, crossings_near_structure_maj, real_crossings_min, crossings_near_structure_min, simulated_crossings_maj,sim_crossings_near_structure_maj, simulated_crossings_min, sim_crossings_near_structure_min, speed, area_sq_km, total_road_length_km_all, total_road_length_km_maj, road_density_all, road_density_maj" > results/results_rr.csv
fi
