...
title(main = paste(individual@info$identity, "Movement Path"),
      ...
      font.main = 1,
      cex.main = 0.85)
lines(major_roads, col = "darkred", lwd = 3)
lines(minor_roads, col = "darkred", lwd = 1)
lines(path_2, col = rgb(0.5, 0.7, 0.99, alpha = 1))
coords_maj <- st_coordinates(crossings_new_maj)  # Extract coordinates for major roads
points(coords_maj[, 1], coords_maj[, 2], col = rgb(0, 0, 0, alpha = 1), pch = 16, cex = 0.5)  # Plot the major roads
coords_min <- st_coordinates(crossings_new_min)  # Extract coordinates for minor roads
points(coords_min[, 1], coords_min[, 2], col = rgb(0, 0, 0, alpha = 1), pch = 16, cex = 0.5)  # Plot the minor roads
